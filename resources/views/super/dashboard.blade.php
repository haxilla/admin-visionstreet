...
<div x-data="{ collapsed: false }" class="h-screen overflow-hidden">
  ...
  >
    <div class="flex justify-center items-center h-20 border-b border-white/10">
      <template x-if="!collapsed">
        <img src="/images/vision-street-square-logo-pixels.png" alt="Full Logo" class="h-12">
      </template>
      <template x-if="collapsed">
        <img src="/images/vision-V-icon.png" alt="V Logo" class="h-8">
      </template>
    </div>
  ...
  <!-- Topbar -->
  <header 
    :class="collapsed ? 'ml-20 w-[calc(100%-5rem)]' : 'ml-64 w-[calc(100%-16rem)]'" 
    class="fixed top-0 right-0 h-16 z-30 bg-white border-b border-gray-200 flex items-center justify-between px-6 transition-all duration-300"
  >
    ...
    <div x-data="{ open: false }" class="relative">
      ...
      <div x-show="open" x-transition 
        ...
        <form method="POST" action="/logout">
          @csrf
          <button type="submit"
                  class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-100 hover:text-red-700 cursor-pointer">
            Log out
          </button>
        </form>
      </div>
