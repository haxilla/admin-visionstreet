    ...
    <div class="flex items-center gap-6 pr-6 text-sm tracking-wider font-russo">
        <div x-data="{ open: false }" class="relative">
          <button
            ...
            class="text-sky-400 hover:text-white transition duration-200 focus:outline-none cursor-pointer"
            aria-label="Settings"
          >
          ...
          >
            ...
            <form method="POST" action="{{ route('logout') }}">
              @csrf
                <button
                type="submit"
                class="w-full text-left px-4 py-2 hover:bg-red-600 hover:text-white text-red-400 transition cursor-pointer">
                    Logout
                </button>
            </form>
          </div>
